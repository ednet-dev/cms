...
void useNewEventStormingPackage() {
  print('\n1. Using the new ednet_event_storming package directly:');
...
void demonstrateBackwardCompatibility() {
  ...
  print('    - Command triggers Event: ${esCommand.triggeredEvents.contains(esEvent)}');
  print('    - Aggregate handles Command: ${esAggregate.commands.contains(esCommand)}');
  print('    - Aggregate produces Event: ${esAggregate.events.contains(esEvent)}');
}

void demonstrateIntegration() {
  print('\n3. Integration between EDNet Core and ednet_event_storming:');
  ...
  print('    - Events count: ${(jsonOutput['domain']['events'] as List).length}');
} 