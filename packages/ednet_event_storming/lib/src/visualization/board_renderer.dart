...
class SvgBoardRenderer implements BoardRenderer {
  ...
  String render(EventStormingBoard board, {Map<String, dynamic> options = const {}}) {
    ...
    // Add board title
    buffer.writeln('<text x="${width / 2 - padding}" y="20" font-family="Arial" font-size="16" text-anchor="middle" font-weight="bold">');
  ...
  void _renderElement(StringBuffer buffer, EventStormingElement element, String type) {
    ...
    buffer.writeln('<g id="${type}_${element.id}" transform="translate($x, $y)">');
    buffer.writeln('<rect width="$width" height="$height" rx="5" ry="5" fill="${element.color}" stroke="#000" stroke-width="1" />');
    
    // Add the element name
    buffer.writeln('<text x="${width/2}" y="20" font-family="Arial" font-size="12" text-anchor="middle" font-weight="bold">');
    buffer.writeln('${_escapeXml(element.name)}');
    buffer.writeln('</text>');
    
    // Add a short description if available
    if (element.description.isNotEmpty) {
      buffer.writeln('<text x="${width/2}" y="40" font-family="Arial" font-size="10" text-anchor="middle">');
      
      // Truncate description if too long
      final truncatedDesc = element.description.length > 30 
...
class HtmlBoardRenderer implements BoardRenderer {
  ...
  String render(EventStormingBoard board, {Map<String, dynamic> options = const {}}) {
    ...
    // Start building HTML
    final buffer = StringBuffer();
    buffer.writeln('<div class="event-storming-board" style="width: ${width}px; height: ${height}px; background-color: #f5f5f5; position: relative;">');
    
    // Add board title
    buffer.writeln('<h2 style="text-align: center; padding-top: 20px;">${_escapeHtml(board.name)}</h2>');
  ...
  void _renderHtmlElement(StringBuffer buffer, EventStormingElement element, String type) {
    final x = element.position.x;
    final y = element.position.y;
