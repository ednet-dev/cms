  ...
  AggregateInsights _analyzeAggregates(EventStormingBoard board) {
    ...
    for (final entry in board.aggregates.entries) {
      ...
      if (aggregate.domainEventIds.isEmpty && aggregate.commandIds.isEmpty) {
        ...
        issues.add('Aggregate "${aggregate.name}" has no associated events or commands');
        recommendations.add('Consider adding events and commands to "${aggregate.name}" or removing it');
      }
      
      // Check for large aggregates (too many events/commands)
      if (aggregate.domainEventIds.length > 10 || aggregate.commandIds.length > 10) {
        largeAggregates.add(aggregate.id);
        issues.add('Aggregate "${aggregate.name}" has many events/commands and might be too large');
        recommendations.add('Consider splitting "${aggregate.name}" into multiple aggregates');
      }
    }
  ...
  EventInsights _analyzeEvents(EventStormingBoard board) {
    ...
    for (final entry in board.domainEvents.entries) {
      ...
      if (event.aggregateId == null) {
        ...
        issues.add('Event "${event.name}" is not associated with any aggregate');
        recommendations.add('Associate "${event.name}" with an appropriate aggregate');
      }
  ...
  CommandInsights _analyzeCommands(EventStormingBoard board) {
    ...
    for (final entry in board.commands.entries) {
      ...
      if (command.aggregateId == null) {
        ...
        issues.add('Command "${command.name}" is not associated with any aggregate');
        recommendations.add('Associate "${command.name}" with an appropriate aggregate');
      }
      ...
      if (command.triggersDomainEventId == null) {
        ...
        recommendations.add('Consider what event "${command.name}" should produce');
      }
    }
  ...
  PolicyInsights _analyzePolicies(EventStormingBoard board) {
    ...
    for (final entry in board.policies.entries) {
      ...
      if (policy.triggeringEventIds.isEmpty) {
        ...
        recommendations.add('Identify what events should trigger "${policy.name}"');
      }
      ...
      if (policy.resultingCommandIds.isEmpty) {
        ...
        recommendations.add('Determine what commands "${policy.name}" should initiate');
      }
    }
  ...
  HotSpotInsights _analyzeHotSpots(EventStormingBoard board) {
    ...
    for (final entry in board.hotSpots.entries) {
      ...
      if (hotSpot.importance >= 4) {
        ...
        issues.add('Hot spot "${hotSpot.title}" has high importance (${hotSpot.importance}) and needs attention');
      }
      
      // Identify unresolved hot spots (no proposed solutions)
      if (hotSpot.proposedSolutions.isEmpty) {
        unresolvedHotSpots.add(hotSpot.id);
        recommendations.add('Brainstorm solutions for hot spot "${hotSpot.title}"');
      }
    }
