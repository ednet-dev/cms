...
void testEDNetCoreTypesTypes(dt.EDNetCoreDomain ednetCoreDomain,
  ...
  group('Testing EDNetCore.Types.Type', () {
    ...
    test('Add type required error', () {
      ...
      var type = dt.CoreType(typeConcept);
    ...
    test('Find type by attribute id', () {
      var randomType = types.random();
      var type = types.singleWhereAttributeId('sequence', randomType.sequence);
      expect(type, isNotNull);
    ...
    test('Find type by required attribute', () {
      var randomType = types.random();
      var type = types.firstWhereAttribute('title', randomType.title);
      expect(type, isNotNull);
    ...
    test('Find type by attribute', () {
      var randomType = types.random();
      var type = types.firstWhereAttribute('started', randomType.started);
      expect(type, isNotNull);
    ...
    test('Select types by required attribute', () {
      var randomType = types.random();
      var selectedTypes = types.selectWhereAttribute('title', randomType.title);
      expect(selectedTypes.isEmpty, isFalse);
      selectedTypes.forEach((se) => expect(se.title, equals(randomType.title)));
    ...
    test('Select types by attribute, then add', () {
      ...
      var type = new dt.CoreType(types.concept);
      type.title = 'enquiry';
    ...
    test('Copy Equality', () {
      ...
      idsEqual = false;
      if (randomType.id?.equals(randomTypeCopy.id!) == true) {
        idsEqual = true;
      }
      expect(idsEqual, isTrue);
    });
    ...
    test('New type action undo and redo', () {
      var typeCount = types.length;
      var type = dt.CoreType(types.concept);
      type.title = 'college';
    ...
    test('Reactions to type actions', () {
      ...
      ednetCoreDomain.startCommandReaction(reaction as ICommandReaction);
      var type = new dt.CoreType(types.concept);
      type.title = 'tent';
...
class TypeReaction implements ICommandReaction {
  bool reactedOnAdd = false;
  bool reactedOnUpdate = false;

  @override
  void react(ICommand action) async {
