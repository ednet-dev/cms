...
void testGlobal_democracyElectoral_systemCitizens(
    Global_democracyDomain global_democracyDomain,
    Electoral_systemModel electoral_systemModel,
    Citizens citizens) {
  ...
  group("Testing Global_democracy.Electoral_system.Citizen", () {
    ...
    test("Select citizens by attribute", () {
      ...
      selectedCitizens.forEach(
          (se) => expect(se.citizenId, equals(randomCitizen.citizenId)));
    ...
    test("Select citizens by attribute, then add", () {
      ...
      citizen.citizenId = 'void';
      citizen.firstName = 'undo';
      citizen.lastName = 'pencil';
    ...
    test("Copy citizens", () {
      ...
      copiedCitizens
          .forEach((e) => expect(e, equals(citizens.singleWhereOid(e.oid))));
    ...
    test("Update citizen non id attribute with failure", () {
      ...
      afterUpdateEntity.citizenId = 'cable';
      expect(afterUpdateEntity.citizenId, equals('cable'));
      // citizens.update can only be used if oid, code or id is set.
      expect(() => citizens.update(randomCitizen, afterUpdateEntity),
    ...
    test("citizen action undo and redo", () {
      ...
      var citizen = Citizen(citizens.concept);
      citizen.citizenId = 'judge';
      citizen.firstName = 'call';
      citizen.lastName = 'consulting';
    ...
    test("citizen session undo and redo", () {
      var citizenCount = citizens.length;
      var citizen = Citizen(citizens.concept);
      citizen.citizenId = 'future';
      citizen.firstName = 'top';
      citizen.lastName = 'up';
      citizens.add(citizen);
      expect(citizens.length, equals(++citizenCount));
      citizens.remove(citizen);
    ...
    test("Citizen update undo and redo", () {
      ...
      var action =
          SetAttributeCommand(session, citizen, "citizenId", 'edition');
      action.doIt();
    ...
    test("Reactions to citizen actions", () {
      ...
      var citizen = Citizen(citizens.concept);
      citizen.citizenId = 'slate';
      citizen.firstName = 'understanding';
      citizen.lastName = 'wave';
      citizens.add(citizen);
      expect(citizens.length, equals(++citizenCount));
      citizens.remove(citizen);
      expect(citizens.length, equals(--citizenCount));
      ...
      var setAttributeCommand =
          SetAttributeCommand(session, citizen, "citizenId", 'meter');
...
void main() {
  ...
  Global_democracyDomain global_democracyDomain =
      repository.getDomainModels("Global_democracy") as Global_democracyDomain;
  assert(global_democracyDomain != null);
  Electoral_systemModel electoral_systemModel = global_democracyDomain
      .getModelEntries("Electoral_system") as Electoral_systemModel;
  assert(electoral_systemModel != null);
  var citizens = electoral_systemModel.citizens;
  testGlobal_democracyElectoral_systemCitizens(
      global_democracyDomain, electoral_systemModel, citizens);
}
