...
// test/project/household/project_household_time_test.dart 
 
import "package:test/test.dart"; 
import "package:ednet_core/ednet_core.dart"; 
import "package:project_household/project_household.dart"; 
 
void testProjectHouseholdTimes( 
    ProjectDomain projectDomain, HouseholdModel householdModel, Times times) { 
  DomainSession session; 
  group("Testing Project.Household.Time", () { 
    ...
    test("From model entry to JSON", () { 
      var json = householdModel.fromEntryToJson("Time"); 
      expect(json, isNotNull); 
 
      print(json); 
      //householdModel.displayEntryJson("Time"); 
      //householdModel.displayJson(); 
      //householdModel.display(); 
    }); 
    ...
    test("Find time by oid", () { 
      var randomTime = householdModel.times.random(); 
    ...
    test("Random time", () { 
      var time1 = householdModel.times.random(); 
      expect(time1, isNotNull); 
      var time2 = householdModel.times.random(); 
    ...
    test("Update time non id attribute with failure", () { 
      ...
      afterUpdateEntity.hours = 3109; 
      expect(afterUpdateEntity.hours, equals(3109)); 
    ...
    test("time action undo and redo", () { 
      ...
      var time = Time(times.concept); 
        time.hours = 6889; 
    var timeProject = householdModel.projects.random(); 
    ...
    test("Time update undo and redo", () { 
      var time = householdModel.times.random(); 
      var action = SetAttributeCommand(session, time, "hours", 9731); 
      action.doIt(); 
    ...
    test("Transaction undo and redo", () { 
      ...
      var time2 = householdModel.times.random(); 
      while (time1 == time2) { 
        time2 = householdModel.times.random();  
      } 
      var action1 = RemoveCommand(session, times, time1); 
      var action2 = RemoveCommand(session, times, time2); 
...
void main() { 
  ...
  var times = householdModel.times; 
  testProjectHouseholdTimes(projectDomain, householdModel, times); 
} 
