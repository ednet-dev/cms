...
// test/project/household/project_household_project_test.dart 
 
import "package:test/test.dart"; 
import "package:ednet_core/ednet_core.dart"; 
import "package:project_household/project_household.dart"; 
 
void testProjectHouseholdProjects( 
    ProjectDomain projectDomain, HouseholdModel householdModel, Projects projects) { 
  DomainSession session; 
  group("Testing Project.Household.Project", () { 
    ...
    test("From model entry to JSON", () { 
      var json = householdModel.fromEntryToJson("Project"); 
      expect(json, isNotNull); 
 
      print(json); 
      //householdModel.displayEntryJson("Project"); 
      //householdModel.displayJson(); 
      //householdModel.display(); 
    }); 
    ...
    test("Find project by oid", () { 
      var randomProject = householdModel.projects.random(); 
    ...
    test("Select projects by attribute, then add", () { 
      ...
      project.budget = 87.28037246092988; 
    ...
    test("Random project", () { 
      var project1 = householdModel.projects.random(); 
      expect(project1, isNotNull); 
      var project2 = householdModel.projects.random(); 
    ...
    test("Update project non id attribute with failure", () { 
      ...
      afterUpdateEntity.name = 'university'; 
      expect(afterUpdateEntity.name, equals('university')); 
    ...
    test("project action undo and redo", () { 
      ...
      var project = Project(projects.concept); 
        project.name = 'course'; 
      project.description = 'cloud'; 
      project.startDate = new DateTime.now(); 
      project.endDate = new DateTime.now(); 
      project.budget = 58.48651796962088; 
    ...
    test("project session undo and redo", () { 
      ...
      project.startDate = new DateTime.now(); 
      project.endDate = new DateTime.now(); 
      project.budget = 33.74484438341696; 
      projects.add(project); 
      expect(projects.length, equals(++projectCount)); 
      projects.remove(project); 
      expect(projects.length, equals(--projectCount)); 
    ...
    test("Project update undo and redo", () { 
      var project = householdModel.projects.random(); 
      var action = SetAttributeCommand(session, project, "name", 'wheat'); 
      action.doIt(); 
    ...
    test("Transaction undo and redo", () { 
      ...
      while (project1 == project2) { 
        project2 = householdModel.projects.random();  
      } 
      var action1 = RemoveCommand(session, projects, project1); 
      var action2 = RemoveCommand(session, projects, project2); 
    ...
    test("Reactions to project actions", () { 
      ...
      var project = Project(projects.concept); 
        project.name = 'call'; 
      project.description = 'guest'; 
      project.startDate = new DateTime.now(); 
      project.endDate = new DateTime.now(); 
      project.budget = 50.5037211844202; 
      projects.add(project); 
      expect(projects.length, equals(++projectCount)); 
      projects.remove(project); 
      expect(projects.length, equals(--projectCount)); 
...
void main() { 
  ...
  var projects = householdModel.projects; 
  testProjectHouseholdProjects(projectDomain, householdModel, projects); 
} 
