...
void testEdnetcoreTasksEmployees(EdnetcoreDomain ednetcoreDomain,
    TasksModel tasksModel, Employees employees) {
  ...
  group("Testing Ednetcore.Tasks.Employee", () {
    ...
    test("Select employees by required attribute", () {
      ...
      selectedEmployees.forEach(
          (se) => expect(se.lastName, equals(randomEmployee.lastName)));
    ...
    test("Copy employees", () {
      ...
      copiedEmployees
          .forEach((e) => expect(e, equals(employees.singleWhereOid(e.oid))));
      copiedEmployees.forEach(
          (e) => expect(e, isNot(same(employees.singleWhereId(e.id!)))));
    ...
    test("Update employee id without try", () {
      ...
      expect(() => randomEmployee.email = 'girl', throws);
    ...
    test("Update employee id with success", () {
      ...
      var attribute =
          randomEmployee.concept.attributes.singleWhereCode("email");
      expect(attribute?.update, isFalse);
      attribute?.update = true;
      afterUpdateEntity.email = 'lifespan';
      expect(afterUpdateEntity.email, equals('lifespan'));
      attribute?.update = false;
      var updated = employees.update(randomEmployee, afterUpdateEntity);
      expect(updated, isTrue);

      var entity = employees.singleWhereAttributeId("email", 'lifespan');
      expect(entity, isNotNull);
      expect(entity!.email, equals('lifespan'));
    ...
    test("Update employee non id attribute with failure", () {
      var randomEmployee = employees.random();
      var afterUpdateEntity = randomEmployee.copy();
      afterUpdateEntity.lastName = 'navigation';
      expect(afterUpdateEntity.lastName, equals('navigation'));
    ...
    test("Employee update undo and redo", () {
      ...
      var action = SetAttributeCommand(session, employee, "lastName", 'meter');
      action.doIt();
    ...
    test("Reactions to employee actions", () {
      ...
      var setAttributeCommand =
          SetAttributeCommand(session, employee, "lastName", 'selfdo');
...
void main() {
  ...
  EdnetcoreDomain ednetcoreDomain =
      repository.getDomainModels("Ednetcore") as EdnetcoreDomain;
  assert(ednetcoreDomain != null);
  TasksModel tasksModel =
      ednetcoreDomain.getModelEntries("Tasks") as TasksModel;
